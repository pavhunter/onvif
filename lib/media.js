...
module.exports = function(Cam) {
	...
	*/
	...
	/**
	 * Set the OSD by Token.
	 * @param {string} [token] Token of the Video Source Configuration, which has OSDs associated with are requested.
	 * If token not exist, request all available OSDs.
	 * @param {Cam~GetOSDsCallback} callback
	 */
	Cam.prototype.setOSD = function(option, callback) {
		if (callback === undefined) { callback = options; options = {}; }
		this._request({
			service: 'media'
			, body: '<soap:Envelope xmlns:soap="http://www.w3.org/2003/05/soap-envelope" xmlns:wsdl="http://www.onvif.org/ver10/media/wsdl" xmlns:sch="http://www.onvif.org/ver10/schema">' +
					'<soap:Header/>' +
					'<soap:Body>' +
						'<wsdl:SetOSD>' +
							'<wsdl:OSD token="' + option.osdToken + '">' +
								'<sch:VideoSourceConfigurationToken>' + option.videoToken + '</sch:VideoSourceConfigurationToken>' +
								'<sch:Type>Text</sch:Type>' +
								'<sch:Position>' +
								'<sch:Type>Custom</sch:Type>' +
								'<sch:Pos  y="' + (option.y || '-1') + '" x="' + (option.x || '-1') + '"/>' +
								'</sch:Position>' +
								'<sch:TextString>' +
								'<sch:Type>Plain</sch:Type>' +
								'<sch:FontSize>' + (option.fontSize || '32') + '</sch:FontSize>' +
								'<sch:FontColor>' +
									'<sch:Color Colorspace="http://www.onvif.org/ver10/colorspace/YCbCr" Z="' + (option.colorZ || '128') + '" Y="' + (option.colorY || '128') + '" X="' + (option.colorX || '16') + '"/>' +
								'</sch:FontColor>' +
								'<sch:PlainText>' + option.text + '</sch:PlainText>' +
								'</sch:TextString>' +
							'</wsdl:OSD>' +
						'</wsdl:SetOSD>' +
					'</soap:Body>' +
